  ...
  return (
    ...
    (
       <div className='flex p-12'>
        {showGptSearch && <select className='mt-5 h-[50%] bg-blue-400 text-white rounded-md' onChange={handleLanguageChange}>
          {SUPPORTED_LANGUAGES.map(lang => <option key={lang.identifier} value={lang.identifier}>{lang.name}</option>)}
      </select>}
        <button onClick={handleGPTSearchClick} className='py-2 px-4 m-5 bg-purple-800 text-white rounded-md'>{showGptSearch ? "Home Page" : "GPT Search"}</button>
